...
modeltwo <- lm_basic(warmth ~ 1, data = hungary)
reg_table(modeltwo, level = 0.95)
```

What do you notice about this confidence interval compared to the original
one? Can you explain why this is the case?

**Answer**:It has a wider range. This may be because it is only the results for warmth of one country and thus will have less variety.
...
```{r}
modeltwo <- lm_basic(warmth ~ 1 + gender, data = hungary)
reg_table(modeltwo, level=0.95)
```
...
**Answer**: There isn't statistical difference beecause the range of the differences is between -1.3 and .971 meaning there could either be a difference or ther couldn't be.
...
```{r}
modelthree <- lm_basic(warmth ~ 1 + fourties, data = age)
reg_table(modelthree, level=0.95)
```
...
**Answer**: Yes, there is, people in the fourties are more warm than people in their thirties by a value between the range of .012 and .18.
