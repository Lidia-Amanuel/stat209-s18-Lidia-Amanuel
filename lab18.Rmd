...
**Answer**: between 54785 and 59091
...
**Answer**: No, there are more values outside of the range given. The values above and below the average range even out.  
...
degree. Does this range intersect the one you had in question 5? Yes, but the range is slightly higher on the lower and higher bounds.

```{r}
modeltwo <- lm_basic(base ~ 1, data = masters)
...
```{r}
modelthree <- lm_basic(age ~ 1, data = london)
  reg_table(modelthree, level = 0.80)

```

Make sure you actually extract the answer here:

**Answer**: Between 33.07 and 34.65 

Describe in words how the proceeding confidence interval should be
interpreted.

**Answer**: The range of the mean for the average age of the victim of a murder in London is 33.07 to 34.65 
